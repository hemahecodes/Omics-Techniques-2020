...
# R scripts generated  Sun Jun 7 08:03:10 EDT 2020
...
gset <- getGEO("GSE58435", GSEMatrix =TRUE, getGPL=FALSE)
if (length(gset) > 1) idx <- grep("GPL570", attr(gset, "names")) else idx <- 1
gset <- gset[[idx]]
...
sml <- sml[order(sml)]
fl <- as.factor(sml)
labels <- c("Control","Turner")

# set parameters and draw the plot
palette(c("#f4dfdf","#dfeaf4", "#AABBCC"))
dev.new(width=4+dim(gset)[[2]]/5, height=6)
par(mar=c(2+round(max(nchar(sampleNames(gset)))/2),4,2,1))
title <- paste ("GSE58435", '/', annotation(gset), " selected samples", sep ='')
